...
export default function MyBlogsPage() {
  ...
  const fetchMyBlogs = async () => {
    ...
    try {
      ...
      const data = await blogService.getAllBlogs({ author: user.id });
      setBlogs(data.blogs);

      // Calculate stats
      const published = data.blogs.filter(b => b.status === 'published' || !b.status).length;
      const drafts = data.blogs.filter(b => b.status === 'draft').length;
      const views = data.blogs.reduce((sum, b) => sum + (b.views || 0), 0);
      const likes = data.blogs.reduce((sum, b) => sum + (b.likes || 0), 0);
  ...
  const filteredBlogs = blogs.filter(blog => {
    ...
    const matchesTab = activeTab === 'all' || blogStatus === activeTab;
  ...
  const handleDuplicate = async (blog) => {
    try {
      const { _id, createdAt, updatedAt, author, ...blogData } = blog;
      const newBlogData = {
        ...blogData,
  ...
  return (
    <MainLayout>
      <section className="py-12 bg-beige-100 dark:bg-primary-700 transition-colors duration-300 min-h-screen">
        <Container>
          {/* Header */}
            ...
            <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-lg font-medium transition-all ${activeTab === 'published'
                    ? 'bg-primary-700 dark:bg-white text-white dark:text-primary-700'
                    : 'bg-white dark:bg-transparent border-2 border-primary-700 dark:border-white text-primary-700 dark:text-white hover:bg-primary-700/10 dark:hover:bg-white/10'
                    }`}
                >
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-lg font-medium transition-all ${activeTab === 'drafts'
                    ? 'bg-primary-700 dark:bg-white text-white dark:text-primary-700'
                    : 'bg-white dark:bg-transparent border-2 border-primary-700 dark:border-white text-primary-700 dark:text-white hover:bg-primary-700/10 dark:hover:bg-white/10'
                    }`}
                >
          ...
          {isLoading ? (
            ...
            <div className="space-y-4">
              {filteredBlogs.map((blog) => (
                <div key={blog._id} className="card p-6 hover:shadow-xl transition-all">
                  <div className="flex flex-col md:flex-row gap-6">
                    {/* Image */}
                    <div className="md:w-48 flex-shrink-0">
                      <img
                        src={blog.coverImage ? `http://localhost:5000/${blog.coverImage}` : 'https://images.unsplash.com/photo-1499750310107-5fef28a66643?w=400&q=80'}
                        alt={blog.title}
                      ...
                      <div className="flex items-start justify-between gap-4 mb-2">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className={`px-2 py-1 text-xs font-semibold rounded-full ${(!blog.status || blog.status === 'published')
                          ...
                          <p className="text-primary-700 dark:text-white/80 mb-3 line-clamp-2" dangerouslySetInnerHTML={{ __html: blog.content }}>
                          </p>
